...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        UIFuncitons.shadow_style(self, self.Class_QF, QColor(255,255,255))
    ...
    def Target_setText(self, num):
        new_n = (num+random.randint(0,num))//3;
        num = str(num)
        new_n = str(new_n)
        self.Target_num.setText(num)

        self.char_label.setText(f"当前车流量: {new_n}")
    ...
    def img_predict(self):
        ...
        print(detections)
        detections.tracker_id = result.boxes.cls.cpu().numpy().astype(int)
        # 画标签
