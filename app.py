...
# 关于：Execution of job "scheduled_function (trigger: interval[0:00:10], next run at: 2024-04-01 12:24:15 CST)" skipped: maximum number of running instances reached (1)
...
@app.route('/alarmList', methods=['POST'])
def get_alarm_list1():
    loc = request.json.get('loc')

    select_sql = f"SELECT id, location, description, threshold, photo, pid, create_time, remark " \
                 f"FROM alarm WHERE location = %s"
    print(loc)
    # 执行查询
    alarm_list = db.get_list(select_sql, (loc,))
...
# 查询车辆出现信息（分页查询）
@app.route('/car_appearance/<int:page>', methods=['GET'])
def get_car_appearance_list(page):
    page_from = int((page - 1) * 10)
    page_to = int(page)*10
    select_sql = f"SELECT id, car_id, appear_time, photo, location, monitor_id " \
                 f"FROM car_appearance LIMIT {page_from}, {page_to}"
    car_list = db.get_list(select_sql)

    # 将datetime对象格式化为字符串
    for item in car_list:
        item['appear_time'] = item['appear_time'].strftime('%Y-%m-%d %H:%M:%S')

    return wrap_ok_return_value(car_list)

# 查询车辆出现信息（分页查询）
@app.route('/car_appearance', methods=['POST'])
def get_car_appearance_car_list():
    # 从请求中获取参数
    color = request.json.get('color')
    make = request.json.get('make')
    # 使用参数化查询来防止SQL注入攻击
    select_sql = "SELECT id FROM car WHERE color LIKE %s AND brand LIKE %s"
    # 执行查询
    car_list = db.get_list(select_sql, ('%' + color + '%', '%' + make + '%'))
    print(car_list)
    if len(car_list)<1:
        return wrap_ok_return_value(car_list)
    ...
    print(id)
    # 使用参数化查询来防止SQL注入攻击
    select_sql2 = "SELECT id, car_id, appear_time, photo, location, monitor_id " \
                  "FROM car_appearance WHERE car_id = %s"

    # 执行查询
    car_list2 = db.get_list(select_sql2, (id,))

    for item in car_list2:
        item['appear_time'] = item['appear_time'].strftime('%Y-%m-%d %H:%M:%S')
    print(car_list2)
    return wrap_ok_return_value(car_list2)
