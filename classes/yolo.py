...
class YoloPredictor(BasePredictor, QObject):
    ...
    def upload_image(self,image_path):
        ...
        # 输出服务器端的响应
        print(f"Uploaded {image_path}: {response.text}")
        # 删除图片文件
        os.remove(image_path)
        print(f"Deleted {image_path}")

    def get_make_and_color_from_server(self,image):
        # 定义服务器端的地址
        server_url = 'http://127.0.0.1:5000'
    ...
    def res_address(self, img_res, result, height, width, model, out):
            ...
            self.frame_id += 1;
            self.tmp_path = 'cache_for_color_and_make'
            # 如果没有识别的：
            ...
            # 如果有识别的
                ...
                #
                #     self.get_make_and_color_from_server(img_res);
    ...
    # 画标签到图像上
    def creat_labels(self, detections, img_box, modele):
        # 要画出来的信息
        # labels_draw = [
        ...
        # ]
        labels_draw = []
        confidences = detections.confidence  # 提取置信度值数组
        class_ids = detections.class_id  # 提取类别 ID 数组
        class_names = detections.data['class_name']  # 提取类别名称数组
        tracker_ids = detections.tracker_id
        # 遍历置信度、类别 ID 和类别名称数组
        for confidence, class_id, class_name,tracker_id in zip(confidences, class_ids, class_names,tracker_ids):
            label = f"OBJECT-ID: {tracker_id} CLASS: {class_name} CF: {confidence:0.2f}"
        ...
        # 遍历置信度、类别 ID 和类别名称数组
        for confidence, tracker_id, class_name in zip(confidences, tracker_ids, class_names):
            label = f"OBJECT-ID: {tracker_id} CLASS: {class_name} CF: {confidence:0.2f}"
