...
class Model:
    ...
    def __init__(self, model: Union[str, Path] = 'best.pt', task=None) -> None:
        """
        Initializes the YOLO model.

        Args:
            model (Union[str, Path], optional): Path or name of the model to load or create. Defaults to 'best.pt'.
            task (Any, optional): Task type for the YOLO model. Defaults to None.
        """
        ...
        if not suffix and Path(model).stem in GITHUB_ASSET_STEMS:
            model, suffix = Path(model).with_suffix('.pt'), '.pt'  # add suffix, i.e. yolov8n -> best.pt
    ...
    def _check_is_pytorch_model(self):
        ...
        if not (pt_module or pt_str):
            raise TypeError(f"model='{self.model}' must be a *.pt PyTorch model, but is a different type. "
                            ...
                            f"'yolo export model=best.pt', but exported formats like ONNX, TensorRT etc. only "
